...
const DEFQuery = (Query: any, C: QGlobal) => {
  ...
  const BOOLQuery = (Query: any, C: QGlobal) => {
    if (C.input !== undefined && C.input?.length !== 0) {
      Query[C.query] = typeof C.queryInput === 'string' ? stringToBoolean(C.queryInput) : C.queryInput;
      return Query[C.query];
    }
  };
  ...
  export const INFQuery = (Query: any, C: QGlobal) => {
    if (C.input === C.expect && C.queryInput !== undefined) {
      Query[C.query] = {
        $in: typeof C.queryInput === 'string' && C.queryInput?.length !== 0 ? C.queryInput : []
      };
      return Query[C.query];
    }
  };
  ...
  export const ORQuery = (Query: any, C: QGlobal) => {
    if (C.input !== undefined && C.input.length !== 0 && Array.isArray(C.queryInput)) {
      const orQueries: any[] = [];
      for (const QueryInput of C.queryInput) {
        if (FilteringFactory.isQGlobal(QueryInput) && QueryInput.queryInput !== undefined) {
  ...
  export const ORFQuery = (Query: any, C: QGlobal) => {
    if (C.input !== undefined && C.input.length !== 0 && Array.isArray(C.queryInput) && C.input === C.expect) {
      const orQueries: any[] = [];
      for (const QueryInput of C.queryInput) {
        if (FilteringFactory.isQGlobal(QueryInput) && QueryInput.queryInput !== undefined) {
          orQueries.push(FilteringFactory.GenerateQuery([QueryInput]));
  ...
  export const EQGRSQuery = (Query: any, C: QGlobal) => {
    if (C.input !== undefined && C.expect !== undefined && C.queryInput !== undefined) {
